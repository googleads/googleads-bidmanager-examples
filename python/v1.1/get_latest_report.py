...
parser = argparse.ArgumentParser(
    add_help=False,
    description="Downloads a report if it has been created in the given "
    "timeframe.",
)
parser.add_argument(
    "--output_directory",
    default=(os.path.dirname(os.path.realpath(__file__))),
    help=("Path to the directory you want to save the report to."),
)
parser.add_argument(
    "--query_id",
    default=0,
...
parser.add_argument(
    ...
    type=int,
    help=(
        "The age a report must be in hours at a maximum to "
        "be considered fresh."
    ),
)
...
def main(doubleclick_bid_manager, output_dir, query_id, report_window):
    if query_id:
        ...
        query = (
            doubleclick_bid_manager.queries()
            .getquery(queryId=query_id)
            .execute()
        )
        try:
            # If it is recent enough...
            if is_in_report_window(
                query["metadata"]["latestReportRunTimeMs"], report_window
            ):
                ...
                print(
                    'No reports for queryId "%s" in the last %s hours.'
        ...
        if "queries" in response:
            ...
            while "nextPageToken" in response and response["nextPageToken"]:
                response = (
                    doubleclick_bid_manager.queries()
                    .listqueries(pageToken=response["nextPageToken"])
                    .execute()
                )
                print_queries(response)
        else:
...
if __name__ == "__main__":
    ...
    if not QUERY_ID:
        try:
            QUERY_ID = int(
                input("Enter the query id or press enter to list queries: ")
            )
        except ValueError:
            QUERY_ID = 0
