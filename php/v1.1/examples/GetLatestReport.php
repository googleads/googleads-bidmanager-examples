    ...
    public function run()
    {
        ...
        if ($queryId === '' || $queryId === 0) {
            ...
            if (isset($result['queries']) && count($result['queries']) > 0) {
                ...
                while (!is_null($result->nextPageToken) && !empty($result->nextPageToken)) {
                    $result = $this->service->queries->listqueries(
            ...
            if (
                $query->metadata->latestReportRunTimeMs >
                (microtime(true) * 1000 - self::CURRENT_REPORT_WINDOW_MS)
            ) {
                // Grab the report.
                file_put_contents(
                    $query->queryId . '.csv',
                ...
                printf('<p>No reports for query Id %s in the last 12 hours.</p>', $queryId);
            }
        }
    }
